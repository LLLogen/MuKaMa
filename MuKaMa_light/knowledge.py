...
class ProductDKInjector(DKInjector):
    ...
    def initialize(self):
        """Initialize spacy"""
        self.nlp = spacy.load('en_core_web_lg', disable=["tagger", "ner", "lemmatizer"])
...
class GeneralDKInjector(DKInjector):
    ...
    def initialize(self):
        """初始化spacy"""
        self.nlp = spacy.load('en_core_web_lg', disable=["tagger", "ner", "lemmatizer"])#采用英文的nlp模型

    def transform(self, entry):
        """Transform a data entry.转换数据条目

        使用NER识别与产品相关的命名实体，并且在序列总标记它们。
        将数字标准化为相同的格式
        ...
        """
        res = ''#储存转换后的结果
        doc = self.nlp(entry, disable=['tagger', 'parser'])#使用spacy进行文本处理，禁用标签和解释器
        ents = doc.ents#获取文档中的命名实体
        start_indices = {}#储存命名实体的起始位置
        end_indices = {}#储存命名实体的结束位置
